...
export async function getLinksBySearchTerm(searchTerm) {
  const sqlQuery =
    "SELECT * FROM links INNER JOIN subjects ON links.subject = subjects.subject_id WHERE links.description iLIKE ($1) OR subjects.subject iLIKE ($1)";
  const parameterValue = [`%${searchTerm}%`];
  const result = await pool.query(sqlQuery, parameterValue);
...
export async function getLinksBySubject(subject) {
  const sqlQuery =
    "SELECT * FROM links INNER JOIN subjects ON links.subject = subjects.subject_id WHERE subjects.subject_id = ($1) ORDER BY likes DESC;";
...
export async function countLikes(id, linksCard) {
  const sqlQuery =
    "UPDATE links SET likes = ($1) WHERE link_id = $2 RETURNING * ;";
